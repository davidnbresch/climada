...
%       =-1: ONLY return stats, i.e. only key fields (lon, lat, peril_ID,
%       units) and hazard.stats added, but fields like intensity not
%       returned. Useful if one wants to obtain stats of a very large
%       hazard set and hence avoid storing more than necessary twice.
%       if store_stats2hazard is a character array, it sets
%       hazard.filename=store_stats2hazard, then store_stats2hazard=-1
%       store hazard to hazard.filename (if it does not exist yet, for
%       safety reasons to avoid overwriting hazard).
% OUTPUTS:
...
if ~isfield(hazard,'stats')
    ...
    [ssi,ssi_sorted,xs_freq,ssi_orig,ssi_sorted_orig,xs_freq_orig]=climada_hazard_ssi(hazard,0);
    hazard.ssi=ssi;
    hazard.ssi_sorted=ssi_sorted;
    hazard.xs_freq=xs_freq;
    hazard.ssi_orig=ssi_orig;
    hazard.ssi_sorted_orig=ssi_sorted_orig;
    hazard.xs_freq_orig=xs_freq_orig;
