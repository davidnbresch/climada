...
% Last Modified by GUIDE v2.5 09-Oct-2016 12:58:36
...
auto_sel_assets=get(handles.checkbox_assets,'Value');

call_from_GUI.axes_left=handles.axes_left;
call_from_GUI.axes_right=handles.axes_right;
[~,~,err_msg]=climada_tc_event_damage_ens(UNISYS_regi,UNISYS_year,UNISYS_name,ens_n,auto_sel_assets,call_from_GUI);
if ~isempty(err_msg),fprintf('%s\n',err_msg);end
...
% --- Executes on button press in checkbox_assets.
function checkbox_assets_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox_assets (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkbox_assets
