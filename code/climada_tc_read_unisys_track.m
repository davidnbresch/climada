...
if sum(index_sametime)>0
    ...
    tc_track.forecast        (index_sametime) = [];
    fprintf('Warning: %i duplicate timestep(s) removed\n',length(index_sametime));
end

% find and delete entries with previous times
index_sametime = find(diff(tc_track.datenum)<0)+1;
if sum(index_sametime)>0
    tc_track.lon             (index_sametime) = [];
    tc_track.lat             (index_sametime) = [];
    ...
    tc_track.datenum         (index_sametime) = [];
    tc_track.forecast        (index_sametime) = [];
    fprintf('Warning: %i backward timestep(s) removed\n',length(index_sametime));
end

% find and delete entries with previous times (and a second time, to be sure)
index_sametime = find(diff(tc_track.datenum)<0)+1;
if sum(index_sametime)>0
    tc_track.lon             (index_sametime) = [];
    tc_track.lat             (index_sametime) = [];
    ...
    tc_track.datenum         (index_sametime) = [];
    tc_track.forecast        (index_sametime) = [];
    fprintf('Warning: %i additional backward timestep(s) removed\n',length(index_sametime));
end
...
if sum(index_sametime)>0
    ...
    tc_track.datenum         (index_sametime) = [];
    tc_track.forecast        (index_sametime) = [];
    fprintf('Warning: %i additional duplicate timestep(s) removed\n',length(index_sametime));
end

% calculate timestep between measurement and the following measurement in hours
...
tc_track_out.forecast             = tc_track.forecast; % added 20151018
